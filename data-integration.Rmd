...
```{r}
ri3k <- clusterRand(clusters.mnn[rescaled$batch==1], colLabels(pbmc3k), mode="index")
ri3k
ri4k <- clusterRand(clusters.mnn[rescaled$batch==2], colLabels(pbmc4k), mode="index")
ri4k
```
...
# For the first batch.
tab <- clusterRand(colLabels(pbmc3k), clusters.mnn[rescaled$batch==1])
heat3k <- pheatmap(tab, cluster_row=FALSE, cluster_col=FALSE,
    col=rev(viridis::magma(100)), main="PBMC 3K probabilities", silent=TRUE)

# For the second batch.
tab <- clusterRand(colLabels(pbmc4k), clusters.mnn[rescaled$batch==2])
