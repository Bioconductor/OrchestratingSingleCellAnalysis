...
library(scran)
rand <- clusterRand(pred.seger$labels, sce.seger$CellType, mode="index")
stopifnot(rand > 0.95)
```
...
# Checking that I'm not just talking shit.
rand <- clusterRand(new.labels, sce.tasic$broad_type, mode="index")
stopifnot(rand > 0.95)
```
...
stopifnot(clusterRand(full.labels, sce.muraro$label, mode="index") < 0.5)
stopifnot(clusterRand(sub.labels, sce.muraro$label, mode="index") > 0.9)
```
