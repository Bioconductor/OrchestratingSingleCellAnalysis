...
As the ADTs are (relatively) deeply sequenced, we can expect non-zero counts for most ADTs in each cell due to contamination (Figure \@ref(fig:detected-ab-hist); if this is not the case, we might suspect some failure of ADT processing for that cell.
...
Filtering uses the outlier-based approach described in Chapter \@ref(quality-control); hard thresholds are more difficult to specify due to experiment-by-experiment variation in the expected coverage of ADTs.
Paradoxically, though, this QC metric becomes less useful in well-executed experiments where there is not enough non-specific binding to obtain consistent coverage of the control ADTs.

```{r detected-con-hist, fig.cap="Distribution of the log-coverage of IgG control ADTs across all cells in the PBMC dataset. The red dotted line indicates the threshold below which cells were removed."}
...
hist(log1p(df.ab2$subsets_controls_sum), col='grey', breaks=50,
    main="", xlab="Log-coverage of controls")
abline(v=log1p(attr(con.discard, "thresholds")["lower"]), col="red", lty=2)
```

The total ADT count is less useful as a QC metric because it is heavily correlated with the biological state of the cell.
The presence of a targeted protein can lead to an order-of-magnitude increase in the total ADT count given the binary nature of most surface protein markers.
Removing cells with low total ADTs would strongly penalize cell types that do not express many (or any) of the selected protein targets, and would represent an extreme case of the issues discussed in Section \@ref(outlier-assumptions).
...
This is inspired by a similar approach for normalization of ChIP-seq data based on background enrichment [@lun2016csaw].
...
In this case, though, this information has already been discarded from the dataset as we only have the counts for cell-containing libraries.
We instead use a proxy profile (Figure \@ref(fig:pbmc-adt-ambient-profile)) constructed by assuming that most cells exhibit ambient-level coverage for each ADT.
This assumption suggests that each ADT should exhibit a bimodal distribution of log-counts across all cells, meaning that we can use all cells in the lower mode to compute the (presumably) ambient concentration.

```{r pbmc-adt-ambient-profile, fig.cap="Inferred relative abundance of each ADT in the ambient solution of the the PBMC dataset."}
# Replace 'ambient' with actual ambient profile, if available.
ambient <- inferAmbience(counts(altExp(sce)))
barplot(ambient, las=2, ylab="Relative ambient ambience")
```

We use a `r Biocpkg("DESeq2")`-like approach to compute size factors based on the ambient profile.
Specifically, the size factor for each cell is defined as the median of the ratios of that cell's counts to the ambient profile.
If the abundances for most ADTs in each cell are ambient-derived, they should be roughly constant across cells;
...
The use of the median protects against the minority of ADTs corresponding to genuinely expressed markers.
