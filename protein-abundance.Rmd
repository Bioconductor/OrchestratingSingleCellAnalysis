...
# TODO: add a subset.cols= method
# TODO: allow people to pass in their own names.
chosen.correlations <- computeCorrelations(sce.main[,chosen], altExp(sce.main)[,chosen])
...
```{r, echo=FALSE}
stopifnot("ENSG00000113088" %in% head(with.pd1$feature1[which(with.pd1$rho > 0)]))
stopifnot("ENSG00000100450" %in% head(with.pd1$feature1[which(with.pd1$rho < 0)]))
```
...
```{r correlation-pd1-blocked, fig.cap="Expression of _IL7R_ with respect to CD127 abundance in each cluster of the PBMC dataset."}
# Using multiple core for speed.
library(BiocParallel)
blocked.correlations <- computeCorrelations(sce.main, altExp(sce.main), 
    block=clusters.adt, BPPARAM=MulticoreParam(4))
...
This uses a PCA to approximate the distances between genes in rank space, followed by a nearest-neighbors approach to identify the top closest genes for each protein marker.

```{r, eval=FALSE}
set.seed(100)
top.correlations <- findTopCorrelations(x=altExp(sce.main), y=sce.main, number=10)
top.correlations
```
