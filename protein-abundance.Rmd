...
The other common metric is the number of ADTs detected (i.e., with non-zero counts) in each cell, but again, this is not as helpful in ADT data compared to RNA data.
...
We re-use the `perCellQCMetrics()` function to compute some metrics from the ADT count matrix.

```{r}
library(scuttle)
controls <- grep("^Ig", rownames(altExp(sce)))
df.adt <- perCellQCMetrics(altExp(sce), subsets=list(controls=controls))
```

As mentioned before, the number of detected ADTs is not very helpful here.
Nonetheless, those cells with very low numbers of detected ADTs are likely to be failures of some kind - so we remove them.
Specifically, we remove cells where the number of detected ADTs is less than or equal to half of the total number of tags.
...
ab.discard <- adt.detected <= n.nonzero/2
summary(ab.discard)
```

This experiment includes isotype control (IgG) antibodies that have similar properties to a primary antibody but lack a specific target in the cell, thus providing a measure of non-specific binding.
...
con.discard <- isOutlier(df.adt$subsets_controls_sum, log=TRUE, type="lower")
summary(con.discard)

hist(log1p(df.adt$subsets_controls_sum), col='grey', breaks=50,
    main="", xlab="Log-total count for controls per cell")
abline(v=log1p(attr(con.discard, "thresholds")["lower"]), col="red", lty=2)
```

<!--
Another potential artifact in CITE-seq data is that of antibody conjugates in the ambient solution.
The contamination of a droplet by a clump of antibodies causes large increase in the counts for all tags in the corresponding cell library.
To identify these, we compute the entropy for each cell from the ADT count matrix.
Higher entropies indicate that the counts are more evenly distributed across tags, consistent with the presence of an antibody conjugate.
By comparison, cells with a small number of highly abundant tags would have low entropies.
We use an outlier-based approach to identify and remove cells with unusually high entropies.

```{r}
# TODO: identify conjugates in DropletUtils.
library(TSCAN)
ent <- perCellEntropy(altExp(sce))
ent.discard <- isOutlier(ent + log(df.adt$sum), type="higher")
summary(ent.discard)

hist(ent)
abline(v=attr(ent.discard, "thresholds")["lower"], col="red", lty=2)
```
-->
...
```{r, echo=FALSE}
stopifnot(all(table(clusters.intersect) > 10))
```
...
For example, in cluster `r of.interest`, we observe that PD-1 abundance is positively correlated with _GZMK_ but negatively correlated with _GZMH_ (Figure \@ref(fig:correlation-pd1-cluster)).

```{r correlation-pd1-cluster, fig.cap=sprintf("Expression of _GZMH_ and _GZMK_ with respect to PD-1 abundance in each cell of cluster %s in the PBMC dataset.", of.interest)}
...
with.pd1 <- chosen.correlations[chosen.correlations$feature2=="PD-1",]
with.pd1 <- with.pd1[order(with.pd1$p.value),]
with.pd1[which(with.pd1$rho > 0),]
with.pd1[which(with.pd1$rho < 0),]

plotExpression(sce.main[,chosen], x="PD-1", show_smooth=TRUE, show_se=FALSE, 
    features=c("GZMH", "GZMK"), swap_rownames="Symbol")
```

```{r, echo=FALSE}
stopifnot("GZMK" %in% head(with.pd1$feature1[which(with.pd1$rho > 0)]))
stopifnot("GZMH" %in% head(with.pd1$feature1[which(with.pd1$rho < 0)]))
```
