...
For example, the most abundant marker in each cell often takes up 40-60% of its total count and is different across cells (Figure \@ref(fig:top-prop-adt-dist)).
...
```{r top-prop-adt-dist, fig.width=10, fig.height=6, fig.cap="Distribution of the proportion of counts taken up by the most abundant marker in each cell, stratified by the identity of the marker. Each boxplot contains the distribution across all cells for which the corresponding marker is the most abundant feature in that cell."}
library(DelayedMatrixStats)
adt.counts <- counts(altExp(sce))
total.count <- colSums(adt.counts)
top.prop <- colMaxs(adt.counts)/total.count
top.marker <- rownames(adt.counts)[max.col(t(adt.counts))]
boxplot(split(top.prop, top.marker), ylab="Proportion of total ADT count")
```
...
This may result in near-identical metrics for all cells (Figure \@ref(fig:cite-detected-ab-hist)) regardless of the efficacy of cDNA capture. 
...
Most counts in an RNA-seq library are assigned to constitutively expressed genes that should not exhibit major changes across subpopulations, allowing us to use the total RNA count as a QC metric that is (somewhat) independent of biological state.
...
As previously mentioned, we expect non-zero counts for most ADTs due to this contamination, so their absence is indicative of a problem during library preparation.
We can then identify droplets that have no estimated contribution from the ambient solution and mark them for removal during QC filtering.
Estimation of the ambient contribution assumes most markers are not actually expressed by any given cell, 
which is generally reasonable for diverse antibody panels that aim to characterize many different cell types.
The calculations themselves are done using the `cleanHashDrops()` function from the `r Biocpkg("DropletUtils")` package, as shown below.

```{r}
controls <- grep("^Ig", rownames(altExp(sce))) # see below for details.
qc.stats <- cleanHashDrops(counts(altExp(sce)), controls=controls)
summary(qc.stats$zero.discard) # libraries removed with no ambient contamination
```

Continuing on the theme of ambient contamination, another potential artifact in CITE-seq data is that of antibody conjugates.
The inclusion of a clump of antibodies in a droplet causes large increase in the counts for all tags in the corresponding library,
which interferes with the downstream analysis by masking the cell's true surface marker phenotype.
We identify affected barcodes as those with unusually high counts for all tags (Figure \@ref(fig:cite-ambient-scale-hist)),
quantified as an ambient "scaling factor" that is closely related to the size factors used for normalization.
The exact threshold is defined by `cleanHashDrops()` using the same outlier-based approach from Section \@ref(quality-control-outlier).

```{r cite-ambient-scale-hist, fig.cap="Distribution of the log-transformed ambient scaling factor to each cell in the PBMC dataset. The red dotted line indicates the threshold above which cells were removed."}
summary(qc.stats$high.ambient)
hist(log2(qc.stats$ambient.scale))
abline(v=log2(attr(qc.stats$high.ambient, "threshold")["higher"]), 
    col="red", lty=2)
```

This experiment also includes isotype control (IgG) antibodies that have similar properties to a primary antibody but lack a specific target in the cell, thus providing a measure of non-specific binding.
Assuming that the magnitude of non-specific binding is constant across cells, we can define low-quality cells as those with unusually low coverage of the control ADTs, presumably due to failed capture or sequencing.
We identify these cells as low outliers (Figure \@ref(fig:cite-total-con-hist)), given that hard thresholds are more difficult to specify due to experiment-by-experiment variation in the expected coverage of ADTs.
...
Helpfully, `cleanHashDrops()` also provides a `discard` field containing the intersection of all ADT-specific filters described above.
...
Observant readers will note that this is the same principle used by `cleanHashDrops()` to estimate the contribution of the ambient solution to each droplet.
...
```{r gzmh-cd8-t, fig.wide=TRUE, fig.asp=0.7, fig.cap="Distribution of log-normalized expression values of several markers in transcript-derived subclusters of a ADT-derived subpopulation of CD8^+^ T cells."}
of.interest <- "1"
markers <- c("GZMH", "IL7R", "LYZ", "KLRB1")
plotExpression(all.sce[[of.interest]], x="subcluster",
    features=markers, swap_rownames="Symbol")
```
...
or in other words, strong separation between cells in any feature set is sufficient to cause the formation of a new cluster.
...
Changes in the analysis strategy for one feature set does not affect the processing of other sets;
...
For example, in cluster `r of.interest`, we observe that CD127 abundance is positively correlated with _LTB_ but negatively correlated with _GZMH_ (Figure \@ref(fig:correlation-cd127-cluster)).

```{r correlation-cd127-cluster, fig.cap=sprintf("Expression of _GZMH_ and _LTB_ with respect to CD127 abundance in each cell of cluster %s in the PBMC dataset.", of.interest)}
...
plotExpression(sce.main[,chosen], x="CD127", show_smooth=TRUE, show_se=FALSE, 
    features=c("LTB", "GZMH"), swap_rownames="Symbol")
```

```{r, echo=FALSE}
stopifnot("LTB" %in% head(sorted.correlations$feature1[which(sorted.correlations$rho > 0)]))
