...
boxplot(split(top.prop, top.marker), ylab="Proportion of total ADT count", las=2)
```
...
If most ADTs for a cell are zero instead, this is indicative of a problem during library preparation.
...
The coverage of these control ADTs serves as a measure of non-specific binding in each cell, and in particular, the presence of protein aggregates.
Antibody aggregates cause large increase in the counts for all tags in the corresponding library,
