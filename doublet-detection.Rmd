...
To this end, spike-in-based size factors from Section \@ref(spike-norm) can be supplied to the `computeDoubletDensity()` function via the `size.factors.content=` argument.
...
For each observed cell, an initial score is computed by combining the fraction of simulated doublets in its neighborhood and its co-expression score based on [@bais2020scds].
...
In contrast, while true doublets often also appear at the fringe of non-doublet clusters, a cluster containing a small proportion of high-scoring cells is probably safe provided that any interesting results are not being driven by those cells (e.g., checking that DE in an interesting gene is not driven solely by cells with high doublet scores). 
While clustering is still required, this approach is more robust than `findDoubletClusters()` to the quality of the clustering as the scores are computed on a per-cell basis.
