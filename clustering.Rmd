...
For example, the `clusterSNNGraph()` function from `r Biocpkg("scran")` can use $k$-means as an initial step to obtain representative centroids that are then subjected to graph-based clustering (Figure \@ref(fig:tsne-kmeans-graph-pbmc)).
...
This is implemented in the `clusterSilhouette()` function from `r Biocpkg("scran")`,
...
indicating that most cells in each cluster have a majority of neighbors from the same cluster.
...
We can quantify the agreement between two clusterings by computing the Rand index with `r Biocpkg("scran")`'s `clusterRand()`.
...
```{r pbmc-coassignment, fig.cap="Coassignment probabilities between each pair of clusters in the reference Walktrap clustering compared to a higher-resolution alternative clustering for the PBMC dataset. Rows and columns of the heatmap represent clusters in the reference clustering."}
...
```{r bootstrap-matrix, fig.cap="Heatmap of coassignment probabilities from bootstrapping of graph-based clustering in the PBMC dataset. Each row and column represents an original cluster and each entry is colored according to the value of the coassignment probability between that pair of clusters."}
