...
Currently the `rowData` slot is also empty.  However, analogous our call to `addPerCellQC()` in the prior section, the function `addPerFeatureQC()` will insert values in the `rowData` slot of our `sce` object, as we can see below:
