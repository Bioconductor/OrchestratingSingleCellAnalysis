...
```{r, echo=FALSE, results="hide"}
library(scran)
stopifnot(all(as.matrix(markers[[4]][c("Ccnb1"),-(1:3)]) > 0.5))
```
...
Cluster 1 mostly consists of G1 cells while the other clusters are predominantly assigned to G2/M, which is broadly consistent with our conclusions from the cyclin-based analysis.
...
gridExtra::grid.arrange(
    plotExpression(sce.ref, features="ENSMUSG00000046323", x="phases"),
...
The main difference from the `r Biocpkg("SingleR")`-based assignment is that additional manual curation was performed on the marker pairs in `cyclone()` to focus on genes with known roles in cell cycle progression.
This can be expected to yield somewhat more accurate results, though there is no reason that the same filtering could not be applied prior to calling `SingleR()`.
