...
This is done based on the per-cell "delta", i.e., the difference of the assigned label's score from the median score within each cell.
...
We set a minimum threshold on the acceptable delta using an outlier-based approach for each label, where deltas that are substantially lower than the majority of deltas for a given label are marked for pruning (Figure \@ref(fig:singler-dist-pbmc)).
This results in the presence of `NA` labels for a small number of cells in the major types.
Pruning with custom parameters can be achieved by explicitly calling the `pruneScores()` function.
...
The former explicitly focuses on aspects of the data that are known to be interesting, simplifying the process of biological interpration.
However, the cost is that the downstream analysis is restricted by the diversity and resolution of the available labels.
We suggest using both strategies and, if major disagreements are present between label and cluster assignments, use them as the basis for further investigation to discover effects that are not captured by the reference.
