...
sce.pbmc$log10_total_counts <- log10(sce.pbmc$total)

# Compute the rank of each cell by decreasing library size
sce.pbmc$total_counts_rank <- rank(-sce.pbmc$total)
...
# Prepare the app
app <- iSEE(sce.pbmc, initial=initial.state)
```
...
```{r}
library(scran)
markers.pbmc.up <- findMarkers(sce.pbmc, sce.pbmc$cluster, 
...
rowData(sce.pbmc) <- cbind(rowData(sce.pbmc), all.p)
```

The next code chunk sets up an app that shows:
...
Moreover, the code chunk below configures the second and third panel to use the gene (i.e., row) selected in the first panel.
