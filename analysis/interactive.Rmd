...
Exploratory data analysis (EDA) and visualization are crucial for many aspects of data analysis, such as quality control, hypothesis generation, result interpretation and comprehension.
...
The shorter panel codes are useful for the configuration of tours, described in the section [Examples of usages for iSEE apps](#isee-examples).
...
## Examples of usages for iSEE apps
...
Another common view is to overlay the library size (in log space) as a color gradient over the result of a dimensionality reduction method such as t-SNE, UMAP, or ivis [@vanDerMaaten2008tsne;@mcInnes2018umap;@szubert2019ivis].
...
Alternatively, it could also indicate the presence of multiple cell types that differ in total RNA content.
...
Users may also change the information displayed in any panel. For instance, users may choose to color data points by percentage of UMI mapped to mitochondrial genes (“pct_counts_Mito”) in the “Reduced dimension plot 1”.
...
x <- DataFrame(
  ...
  lapply(markers.pbmc.up, function(x){x[rownames(sce.pbmc), "log.FDR"]})
  )
...
The next code chunk launches an app that shows:
...
Rather than sharing analysis results via multiple versions of static figures and reports, it is often helpful to share an interactive version of a plot, allowing the other involved parties to independently explore the data in further depth.
...
The typical way of disseminating analysis results from single-cell data is as static figures in published papers and reports.
These are unlikely to capture the full information content of a complex scRNA-seq data set, and do not allow further exploration by the reader.
...
With `iSEE`, such interactive exploration can be guided by small guided tours - step-by-step walkthroughs of the different panels with pointers to facilitate their interpretation, from the authors' point of view.
At any time, the viewer is still free to leave the provided tour and explore the data from their own perspective.
All that is needed to add a tour to an `iSEE` instance is a data frame with two columns; the first column containing the UI element to be pointed to in each step of the tour, and the second one containing the text to display at that step.
This data frame must then be provided to the `iSEE()` function via the `tour` argument. Examples of tours can be found in https://github.com/ltla/iSEE2018.
...
- Source code: https://github.com/csoneson/iSEE
- Deployed examples: 
