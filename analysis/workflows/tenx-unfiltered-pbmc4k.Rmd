...
top.hvgs <- getTopHVGs(dec.pbmc, prop=0.1)
```
...
set.seed(10000)
sce.pbmc <- denoisePCA(sce.pbmc, subset.row=top.hvgs, technical=dec.pbmc)
...
as.data.frame(marker.set[1:40,1:2])
```
...
topset <- rownames(marker.set)[1:40]
