...
var.fit <- trendVar(sce.zeisel)
var.out <- decomposeVar(sce.zeisel, var.fit)

# Taking 10% of the genes with the top biological components.
ordered.genes <- rownames(var.out)[order(var.out$bio, decreasing=TRUE)]
chosen.hvgs <- head(ordered.genes, 0.1 * nrow(var.out))
```
