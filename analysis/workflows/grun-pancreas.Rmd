...
When computing the median and MAD, we exclude donors for which the assumption of a majority of high-quality cells seems to be violated (Figure \@ref(fig:unref-grun-qc-dist); the resulting threshold on each metric is then applied to all cells.
...
```{r unref-grun-qc-dist, fig.wide=TRUE, fig.cap="Distribution of each QC metric across cells in the Grun pancreas dataset. Each point represents a cell and is colored according to whether that cell was discarded."}
