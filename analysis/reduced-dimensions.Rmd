...
Earlier PCs may also be driven by internal variation within large subpopulations, requiring us to retain more PCs than expected to resolve minor subpopulations.
To deal with these issues, we use a heuristic approach that uses the number of clusters as a proxy for the number of subpopulations.
The `getClusteredPCs()` approach performs graph-based clustering (Chapter \@ref(clustering)) to identify the number of clusters across a range of choices for $d$.
We then set $d$ to the number of PCs that yields the largest number of clusters that is no more than $d+1$ (Figure \@ref(fig:cluster-pc-choice)).
This attempts to capture as many distinct (putative) subpopulations as possible by retaining biological signal in later PCs - up until the point that the additional noise reduces resolution - while respecting the theoretical constraints on the minimum number of PCs required to resolve those subpopulations.
