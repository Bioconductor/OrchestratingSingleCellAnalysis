...
Non-negative matrix factorization (NMF) involves approximating a matrix by the product of two lower-rank matrices,
...
However, the NMF coordinates are more interpretable as large values unambiguously represent greater expression of genes in the corresponding factor; this cannot be said of PCA coordinates, for which negative values could be caused by higher expression if the corresponding entry of the rotation vector is negative.
...
To demonstrate the application of NMF, we take the top 10 factors from the @zeisel2015brain dataset using the `runNMF()` function from `r Biocpkg("scater")` (powered by the `r CRANpkg("NNLM")` package).
...
However, the main benefit of NMF over PCA is that we can directly study the individual factors based on their contributing genes in the basis matrix.
(The product of the basis matrix and the per-cell coordinate matrix forms the approximation to the original input.)
This allows us to quickly characterize cellular heterogeneity and its driver genes (Figure \@ref(fig:heat-nmf-zeisel)).

```{r heat-nmf-zeisel, fig.wide=TRUE, fig.asp=0.5, fig.asp="Heatmaps of the NMF results in the Zeisel brain dataset. Each column represents a factor while each row represents either a cell (left) or a gene (right)."}
set.seed(101001)
...
NMF provides a fast alternative to the relatively circuitous process of clustering (Chapter \@ref(clustering)) and marker gene detection (Chapter \@ref(marker-detection)) for identifying cell populations.
However, it could also be argued that the NMF output is somewhat more difficult to interpret than conventional marker genes for distinguishing between subpopulations.
...
As such, we tend to prefer PCA for general-purpose dimensionality reduction to denoise and compact the data.
That said, NMF can be a powerful tool in more targeted applications where interpretability of the dimensions is important;
one such example is provided in Chapter \@ref(cell-type-annotation).
