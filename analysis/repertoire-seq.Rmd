...
We can gain some insights into the immune activity of each cluster by counting the number of expanded clonotypes as shown below for the $\alpha$-chain.
...
We can quantify this observation by computing diversity indices for clonotypes within each cluster.
...
In practice, it is often better to perform the clonotype analysis on the $\beta$ chain as this is the preferred source of information regarding clonotype identity (see the next Section for more details).
...
For the clonotype analysis, there is little benefit from using `per.cell.tcr` as the clonotypes have already been defined based on information from both chains.
Typically, clonotypes are defined based on identity in the $\beta$ chains, either in the CDR3 region only or across the entire sequence.
...
```{r}
# Demonstrating that they are, in fact, the same.
...
We can count the number of cells for combinations of genes and test for differences in gene usage between clusters.
...
heavy.test <- testGeneComboCountsPairwise(heavy.counts[,has.igh])
