...
plotExpression(altExp(sce.pbmc)[,of.interest], 
    ...
    other_fields="label") + facet_wrap(~cluster, ncol=1)
```
...
ave.markers <- sumCountsAcrossCells(altExp(sce.pbmc), colLabels(sce.pbmc),
    exprs_values="logcounts", average=TRUE)
stopifnot(diff(assay(ave.markers)["CD8a", c(tclust.1, tclust.2)]) < -4)
stopifnot(diff(assay(ave.markers)["CD4", c(tclust.1, tclust.2)]) > 5)
```
...
df.A <- extractDataFrame(sce.pbmc$TRA, cluster=colLabels(sce.pbmc))
...
for (i in properties) {
    prop <- colnames(aap)[i]
    current <- split(aap[,prop], colLabels(aap))
