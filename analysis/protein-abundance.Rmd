...
```{r gzmh-cd8-t, fig.wide=TRUE, fig.asp=0.5, fig.cap="Distribution of log-normalized expression values of _GZMH_ (left) and _GZHK_ (right) in transcript-derived subclusters of a tag-derived subpopulation of CD8^+^ T cells."}
of.interest <- "12"
...
stopifnot(length(all.markers[[chosen.cluster]])==2L)
stopifnot("ENSG00000100450" %in% rownames(all.markers[[chosen.cluster]][[1]])[1:10])
stopifnot("ENSG00000113088" %in% rownames(all.markers[[chosen.cluster]][[2]])[1:10])
```
...
In more interesting applications, protein targets are chosen that reflect some functional activity rather than cell type.
...
of.interest <- 18
pheatmap(as.matrix(markers[[of.interest]][,-c(1:3)]), 
    breaks=seq(-3, 3, length.out=101))
```

```{r, echo=FALSE, results="hide"}
stopifnot(identical(chosen.cluster, of.interest))
stopifnot(unlist(markers[[of.interest]]["PD-1",][-c(1:3)]) > 1)
```
