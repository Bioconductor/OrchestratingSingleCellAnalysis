...
```{r detected-ab-hist, fig.cap="Distribution of the number of detected ADTs across all cells in the PBMC dataset."}
...
Indeed, composition biases are arguably more severe in ADT data than gene expression data as (i) the nature of the selection of protein targets enriches for features that are more likely to be differentially abundant across the population, and (ii) any increases in protein abundance manifests as a large increase to the library size.
    ...
    The geometric mean is a reasonable estimator of the scaling biases for large counts, and it mitigates the effects of composition biases by dampening the effects of one or two highly abundant proteins.
    While more robust than the ADT library size factors, these geometric mean-based factors are still not entirely correct and will progressively become less accurate as protein abundance increases.
...
This is loosely based on a similar approach for normalization of ChIP-seq data based on background enrichment [@lun2016csaw].
    ...
    In this case, though, this information has already been discarded from the provided dataset; instead, we use a proxy profile constructed from the average of all cells, under the assumption that all cell types are equally likely to contribute to the ambient profile.
...
We demonstrate below by looping over all of the ADT-derived clusters and subclustering on gene expression (Figure \@ref(fig:subcluster-stats)).
...
```{r subcluster-tag-dist, fig.asp=0.5, fig.wide=TRUE, fig.cap="Distribution of log-normalized abundances of all ADTs in each subcluster of the CD8^+^ T cell population."}
...
For example, if PD-1 was a functional readout for something - say, T cell exhaustion [@pauken2015overcoming] - we might be interested in its upregulation in cluster `r chosen.cluster`.
...
stopifnot(unlist(of.interest["PD-1",][-c(1:3)]) > 1)
```
