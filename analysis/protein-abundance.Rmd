...
As in Chapter \@ref(normalization), we assume that these are scaling biases and we compute tag-specific size factors to remove them.
...
At first glance, this appears to be a strong assumption - the target proteins were specifically chosen as they exhibit interesting heterogeneity across the population, such that a non-DE majority across tags would be unlikely.
...
This is loosely based on a similar approach for normalization of ChIP-seq data based on background enrichment [@lun2016csaw].
    ...
    If possible, we would obtain an estimate of the ambient profile from the barcodes that were identified as empty droplets (Section \@ref(qc-droplet)).
    However, this information has already been discarded from the provided dataset, so we instead use a proxy profile constructed from the average of all cells under the assumption that all cell types are equally likely to contribute to the ambient profile.
    ...
    These DESeq-like size factors exhibit some subpopulation-driven differences from the library size factors (Figure \@ref(fig:comp-bias-norm)), consistent with the presence of composition biases due to differential abundance of the targeted proteins across subpopulations.
    ...
    g <- buildSNNGraph(tagdata, k=20, d=NA) # no need for PCA.
...
3. Some experiments will include antibodies to uninteresting "control" proteins such as MHC components that are constitutively expressed on every cell.
...
We demonstrate this approach below by pretending that all immunoglobins are control proteins, though obviously one should use something more sensible in practice.
...
Running `logNormCounts()` will then perform scaling normalization and log-transformation for both the endogenous transcripts and the tags.
