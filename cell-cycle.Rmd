...
```{r cell-cycle-regression, fig.width=10, fig.height=6, fig.cap="PCA plots before and after regressing out the cell cycle effect in the 416B dataset, based on the phase assignments from `cyclone()`. Each point is a cell and is colored by its inferred phase."}
library(batchelor)
...
One suspects, though, that the magnitude of the scores will be even more sensitive to any non-orthogonalities between the cell cycle and other biological processes.

```{r cell-cycle-regression2, fig.width=10, fig.height=6, fig.cap="PCA plot on the residuals after regression on the cell cycle phase scores from `cyclone()` in the 416B dataset. Each point is a cell and is colored by its inferred phase."}
...
plotPCA(reg.nocycle2, colour_by=I(assignments$phases), shape_by=I(relabel)) + scaled
```
...
(In this case, the cell cycle phase for each cell is known but the same procedure can be applied to the inferred phases from, e.g., `cyclone()`.)
...
```{r leng-nocycle, fig.width=10, fig.height=6, fig.cap="PCA plots of the Leng ESC dataset, generated before and after removal of cell cycle-related genes. Each point corresponds to a cell that is colored by the sorted cell cycle phase."}
library(scRNAseq)
...
not.cycle.markers <- !rownames(dec.leng) %in% top.cycle.markers 

# And repeating the PCA without them.
top.hvgs2 <- getTopHVGs(dec.leng[not.cycle.markers,], n=1000)
