...
library(scran)
rand <- clusterRand(singler.assignments$labels, assignments$phases, mode="index")
stopifnot(rand > 0.8)
```
...
Similarly, for functions that support blocking, we can use the cell cycle phase assignments as a blocking factor.
...
That said, we do not consider adjusting for cell cycle to be necessary in routine scRNA-seq analyses.
...
Otherwise, regression would remove interesting signal if cell cycle activity varied across clusters or conditions. 
...
We suggest only performing cell cycle adjustment on an as-needed basis in cases with clear (and unwanted!) cell cycle effects.
Moreoever, regression should be applied separately to the subset of the dataset for each subpopulation or cluster,
which reduces the risk of interference from differences in cell cycle behavior across clusters.
Any characterization of gene expression should use the uncorrected data with blocking (Section \@ref(using-corrected-values)), 
which is a sanity check that protects against distortions introduced by the adjustment.

Alternatively, users may consider just excluding cell cycle-related genes from the downstream analysis.
This removes the immediate contributors to the cell cycle effect without requiring any assumptions about orthogonality.
The downside of this approach is that it will not remove the cell cycle effect in genes with no annotated role in the cell cycle,
as demonstrated in Figure \@ref(fig:leng-nocycle) with the @leng2015oscope dataset containing phase-sorted ESCs.
However, in such cases, there is ambiguity over whether that effect is truly due to the cell cycle or from some other (interesting) biological process that happens to be correlated with the cell cycle - say, cell fate decisions [@soufi2016cycling].

```{r leng-nocycle, fig.cap="PCA plot of the Leng ESC dataset, generated after removal of cell cycle and division related genes. Each point corresponds to a cell that is colored by the sorted cell cycle phase."}
library(org.Hs.eg.db)
division.genes <- select(org.Hs.eg.db, keys=c(
        "GO:0008283", # cell proliferation 
        "GO:0051301", # cell division
        "GO:0007049", # cell cycle
        "GO:0006278"  # DNA replication
    ),
    keytype="GOALL", column="SYMBOL")
length(division.genes)

# Performing an analysis without the cell cycle-related genes.
library(scRNAseq)
sce.leng <- LengESCData()
leftovers <- setdiff(rownames(sce.leng), division.genes[,"SYMBOL"])
