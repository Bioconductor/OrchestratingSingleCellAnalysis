...
This is not uncommon with, e.g., increased proliferation of T cells upon activation [@richard2018tcell] and changes in cell cycle phase progression across developmental stages [@roccio2013predicting].
...
The obvious downside is that any correlated processes will also be removed; regression is also capable of introducing spurious signal that interferes with any interpretation of subtle variation.

```{r cell-cycle-regression, fig.width=10, fig.height=6, fig.cap="PCA plots before and after regression of the cell cycle effect in the 416B dataset. Each point is a cell and is colored by its inferred phase."}
library(batchelor)
...
```{r cell-cycle-contrastive, fig.width=10, fig.height=10, fig.cap="$t$-SNE plots for the 416B dataset before and after contrastive PCA. Each point is a cell and is colored according to its inferred cell cycle phase."}
...
con.out <- scPCA(
    ...
    alg="rand_var_proj", penalties=0, n_centers=5, n_eigen=10) 
...
gridExtra::grid.arrange(
    ...
    plotTSNE(sce.416b, colour_by="phenotype"),
    plotTSNE(sce.con, colour_by=I(assignments$phases)) + ggtitle("After (416b)"),
    plotTSNE(sce.con, colour_by="phenotype"), 
    ncol=2
)
```
...
### Removing annotated cell cycle genes
...
go.genes <- select(org.Hs.eg.db, keys="GO:0007049", # cell cycle (GO)
    keytype="GOALL", column="ENSEMBL")[,"ENSEMBL"]

library(reactome.db)
rct.genes <- select(reactome.db, keys="R-HSA-1640170", # cell cycle (REACTOME)
...
plotPCA(sce.nocycle, colour_by="Phase") + 
    geom_point(aes(colour="Phase"), pch=21, colour="grey")
```

The persistence of this separation is driven by differential expression in genes without any direct role in cell cyle progression, possibly indicative of a correlated biological process - or, of course, a sorting artifact.
