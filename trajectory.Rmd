...
Once we have constructed a trajectory, the next step is to interpret the underlying biology based on its DE genes.
...
The overall strategy is to fit a linear model to the per-gene expression with respect to pseudotime,
...
We use `testLinearModel()` to fit a natural spline with 5 degrees of freedom to the expression of each gene,
...
```{r}
# Considering cells assigned to the first path through the TSCAN MST:
first.pseudo <- tscan.pseudo[,1]
keep <- !is.na(first.pseudo) & colLabels(sce.nest)!="7"
first.pseudo <- first.pseudo[keep]
sub.y <- logcounts(sce.nest)[,keep]

# Testing for (potentially non-linear) associations in expression:
library(scran)
basis <- splines::ns(first.pseudo, 5)
design <- model.matrix(~basis)
pseudo1 <- testLinearModel(sub.y, design=design, coefs=2:ncol(design))
...
This yields a quick summary of the overall direction of change (i.e., positive or negative);
in contrast, the magnitude and sign of the spline coefficients cannot be easily interpreted.

```{r}
design.lfc <- model.matrix(~first.pseudo)
pseudo1.lfc <- testLinearModel(sub.y, design=design.lfc)

# Collecting desired annotation and statistics into one DataFrame:
pseudo1.final <- cbind(
    rowData(sce.nest)[,"SYMBOL",drop=FALSE],
    pseudo1.lfc[,"logFC",drop=FALSE], 
    pseudo1[,c("p.value", "FDR")])

# Ordering by the most significant genes on the ANOVA p-values:
sorted <- pseudo1.final[order(pseudo1.final$p.value),]
```
...
```{r, echo=FALSE, results="hide"}
stopifnot(all("Mpo", "Plac8") %in% head(up.left$SYMBOL, 10))
```
...
It is worth pointing out that the $p$-values here should be taken with some skepticism.
The same fundamental problems described in Section \@ref(p-value-invalidity) remain;
