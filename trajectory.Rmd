...
Some extra thought is required to deal with reads spanning exon-intron boundaries, as well as reads mapping to genomic regions that can be either intronic or exonic, depending on the considered transcript isoform.
...
```{r}
# TODO: set the location= default to TRUE.
library(scRNAseq)
...
Alternatively, if the standard exonic count matrix was available, we could just use it directly in these steps, and restrict the role of the spliced/unspliced matrices to the velocity calculations.
The latter approach may be logistically helpful when adding RNA velocity to an existing analysis, such that the prior steps (and the interpretation of their results) do not have to be repeated.
...
velo.out <- scvelo(sce.sperm, assay.X="spliced", use.dimred="PCA")
velo.out
```

The estimated velocities can be embedded and visualized in any low-dimensional representation of the cells. 
`r basilisk::PyPiLink("scvelo")` also returns an estimate of the pseudotime for each cell.

```{r}
embedded <- embedVelocity(reducedDim(sce.sperm, "TSNE"), velo.out)
grid.df <- gridVectors(reducedDim(sce.sperm, "TSNE"), embedded)
sce.sperm$pseudotime <- velo.out$velocity_pseudotime

library(ggplot2)
plotTSNE(sce.sperm, colour_by="pseudotime") +
...
library(TSCAN)
mst <- createClusterMST(sce.sperm, use.dimred="PCA", outgroup=TRUE)
line.data <- reportEdges(sce.sperm, mst=mst, use.dimred="TSNE")
plotTSNE(sce.sperm, colour_by="label") + 
    geom_line(data=line.data, mapping=aes(x=dim1, y=dim2, group=edge))
```

```{r}
boxplot(split(sce.sperm$pseudotime, clusters))
```
...
scVelo returns also a 'latent time', which is intended to capture the cell's 'internal clock': https://github.com/theislab/scvelo/issues/89#issuecomment-558217638
