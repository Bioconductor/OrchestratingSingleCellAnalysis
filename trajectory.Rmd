...
We will not attempt to cover the diversity of options available for model fitting in this section.
Instead, to demonstrate the relevant principles,
we will use the simplest strategy of fitting a linear model to the log-expression values 
with respect to (a non-linear function of) the pseudotime.
More complex approaches involving generalized additive models
are available in the `r Biocpkg("tradeSeq")` and `r Biocpkg("monocle")` packages;
interested readers are referred to their documentation.
...
stopifnot(sum(grepl("Ig", head(pseudo$SYMBOL[o]))) > 2)
```

To simplify the results, we will repeat our analysis after filtering out cluster `r offender`.
This cluster seems to contain a set of B cell precursors that are located at one end of the trajectory,
causing immunoglobulins to dominate the set of DE genes and masking more subtle effects.
...
stopifnot(sum(grepl("Ig", head(pseudo$SYMBOL[o]))) == 0)
```
...
```{r, echo=FALSE, results="hide"}
stopifnot(all("Hlf", "Ltb") %in% head(up.right$SYMBOL, 10))
```

It is worth pointing out that the magnitudes of these $p$-values should be treated with some skepticism.
