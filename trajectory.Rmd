...
```{r}
library(scRNAseq)
sce.sperm <- HermannSpermatogenesisData(strip=TRUE)
...
# Could also use velo.out$root_cell here, for a more direct estimate.
...
The use of unspliced counts increases the sensitivity of the analysis to unannotated transcripts (e.g., microRNAs in the gene body), intron retention events, annotation errors or quantification ambiguities [@soneson2020preprocessing] that could interfere with the velocity calculations.
...
- The velocities at any given coordinate of the expression space are constant with respect to real time.
This is necessary to interpret the velocity pseudotime as a proxy for real time, 
otherwise there's no reason to believe that the current movement of cells reflects their history.
(Obviously, these cells were all collected at the same time, so the pseudotime cannot be a direct proxy for real time without this assumption.)
Alternatively, one could consider the pseudotime as a proxy for the age of the cells, 
but scVelo also returns a 'latent time' to capture the cell's 'internal clock' (https://github.com/theislab/scvelo/issues/89#issuecomment-558217638) 
so even there we have multiple potential interpretations.
