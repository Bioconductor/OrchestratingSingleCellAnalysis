...
where practitioners can root a tree using outgroups or midpoints.
However, the problem is fundamentally different; the ancestral nodes are not directly observed in phylogenetic trees
whereas all states of a trajectory are present in a single-cell dataset.
This precludes the use of existing methods (e.g., midpoint rooting makes little sense for a trajectory)
and requires some more bespoke methods that will be discussed below.
