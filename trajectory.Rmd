...
To construct an ordering, we extrapolate from the vector for each cell to determine that cell's future state.
Roughly speaking, if a cell's future state is close to the observed state of another cell, we can place the former behind the latter to obtain a "velocity pseudotime" (Figure \@ref(fig:tsne-hermann-velocity)).
This yields information about directionality without the need to explicitly define a root in our trajectory.
...
More complex analyses based on 
