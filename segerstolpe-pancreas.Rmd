...
suggests that should have called `fastMNN()` at some point.
...
This yields a more coherent set of clusters in Figure \@ref(fig:unref-seger-tsne-correct)
where each cluster contains contributions from all donors.
...
colLabels(corrected) <- clusterSNNGraph(corrected, use.dimred="corrected")
...
To keep things simple, we will use the author-provided annotation rather than figuring out the cell type for each of our clusters above.
...
This is because `voom()` can use sample weights to better account for the variation in the number of cells 
and hence precision of each pseudo-bulk profile.
We see that insulin (ENSG00000254647) is downregulated in beta cells in the disease state, which is sensible enough.
...
We also create some diagnostic plots.
...
# Easiest just to re-run it with plot=TRUE than
# to make the plot from 'v.beta'.
